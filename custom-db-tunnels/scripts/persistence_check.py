...
"""Persistence smoke test for the custom-db-tunnels lab.

The script connects to the gateway on `ingress_net`. The gateway then reaches
custom nodes through `CLOUDFLARE_URL_1..3`; the script itself never receives DB
node Docker addresses.
"""
...
LAB_NAME = os.environ.get("LAB_NAME", "custom-db-tunnels")
...
def main() -> int:
    ...
    if matches < int(os.environ.get("MIN_OBSERVED_REPLICAS", "1")):
        return fail({"error": "new value was not observed", "matches": matches, "write": write_payload, "after": after_payload})
