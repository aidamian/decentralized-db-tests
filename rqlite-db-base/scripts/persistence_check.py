...
"""Persistence smoke test for the rqlite-db-base lab.

The script runs as the `persistence-client` compose service and uses only the
brokered gateway. Repeated runs write an incrementing counter to a stable key so
the `_volumes/rqlite-db-base/workerN/...` bind mounts are visible.
"""
...
LAB_NAME = os.environ.get("LAB_NAME", "rqlite-db-base")
